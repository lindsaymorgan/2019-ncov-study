...
record=pd.read_csv(f'province-day-summary-2020-02-18.csv')
record1=pd.pivot_table(record, values='province_confirmedCount', index='provinceName',
                    columns='updateTime')
record1['provinceName']=record1.index

# record1['provinceName-en'] = record1.apply(lambda row: pinyin(f'{row["cityName"]}').capitalize(), axis = 1)
#
...
record1.to_csv(f'province-pivot-day-summary-{today}.csv',index=0,encoding='utf-8-sig',sep=',')