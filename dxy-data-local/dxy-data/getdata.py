...
def get_html_page():
    ...
    for item in newitems:
        ...
        if data['provinceName'] in ['北京市', '上海市', '重庆市', '天津市']:
            ...
            city = city.drop(columns=['locationId'], errors='ignore')
            frame_province = frame_province[list(city.columns)]
            city = city.append(frame_province, ignore_index=True)



    city.drop(['deadCount','locationId'], inplace=True, axis=1, errors='ignore')
    ...
    province.drop(['currentConfirmedCount'],inplace=True,axis=1)
    ...
    country['updateTime']=today
    country=country.drop(['provinceName'])
    country.index = ['country_confirmedCount', 'country_curedCount', 'country_suspectedCount','updateTime']
