...
raw_data=pd.read_csv(f'rawdata-{today}.csv')
...
raw_data1 = pd.pivot_table(raw_data, index=[raw_data['updateTime'].dt.date-datetime.timedelta(days=1), 'provinceName'], values=['province_confirmedCount', 'province_suspectedCount', 'province_curedCount','province_deadCount'], aggfunc=np.max)
raw_data1=pd.DataFrame(raw_data1.to_records())
raw_data1=raw_data1[raw_data1['provinceName']!='澳门']
...
raw_data1_old=pd.read_csv(f'../dxy-data/nice-dxy-data/province-day-summary-{former}.csv')
...
# raw_data3_old=pd.read_csv(f'../dxy-data/nice-dxy-data/country-day-summary-{former}.csv')
# raw_data3=pd.concat([raw_data3,raw_data3_old])
...
city=raw_data2.rename(columns={'province_confirmedCount':'city_confirmedCount','province_curedCount':'city_curedCount','province_suspectedCount':'city_suspectedCount','province_deadCount':'city_deadCount'})
...
raw_data2 = pd.pivot_table(raw_data, index=[raw_data['updateTime'].dt.date-datetime.timedelta(days=1), 'cityName'], values=['provinceName','city_confirmedCount', 'city_suspectedCount', 'city_curedCount', 'city_deadCount'], aggfunc=np.max)
...
raw_data2_old=pd.read_csv(f'../dxy-data/nice-dxy-data/city-day-summary-{former}.csv')
