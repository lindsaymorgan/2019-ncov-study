...
latent=5
result=pd.read_csv(f'latency{latent}_city_infection_I_D_{today}.csv')
# data=pd.read_csv(f'../../dxy-data/nice-dxy-data/city-pivot-day-summary-{today}.csv')
result=result[(result['infection_rate']>=0)]
...
plt.savefig(f'latency{latent}-P0-all-cities-{today}.jpg', bbox_inches='tight')
plt.show()