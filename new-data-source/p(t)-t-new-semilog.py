...
data=pd.read_csv(f'./agged-record-data/city-confirmed-pivot-day-summary-{today}.csv')
# day=np.log10(list(range(1,(datetime.datetime.today().date()-datetime.date(2020,1,24)).days)))
day=list(range(0,(today-datetime.date(2019,12,1)).days-1))
cut=8
...
for r,pl in enumerate(['合肥','信阳','蚌埠','南昌','哈尔滨']):
    ...
    popt=np.polyfit(list(compress(day,valid))[cut:], list(compress(v,valid))[cut:],1)
    p1 = np.poly1d(popt)
