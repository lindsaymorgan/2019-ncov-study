...
start_day = '2020-01-24'
except_day = '2020-02-29'
day_plus = 63
w=0.3
...
def final_result(y_real, c_0, f_0):
    ...
    txt_ab_best = open('分开最佳ab结果-%s-%s.txt' % (title, except_day), 'w')
    ...
    print(y_best)
    df_y = pd.DataFrame({'y_best_%s' % title: y_best, 'f_best_%s' % title: f_best})
    df_y.to_excel("分开最佳y结果-%s-%s.xlsx" % (title, except_day))
    ...
    #plt.show()
    plt.savefig("%s-%s.png" % (title,except_day))

date_list_1 = return_day_list(start_day, except_day)
date_list_2 = return_day_list(start_day, str(except_day_plus))
df_0 = pd.read_excel('C:\\Users\\csr-lsy\\Desktop\\假期论文\\项目\\%s-DXYArea_result.xlsx' % except_day)
y_real_pre = df_0['武汉']
